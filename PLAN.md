...
| M3 wk A (FastAPI fundamentals) | 📗 | ⬜ | Days 1–5: HTTP/REST, FastAPI, Pydantic, DI + Postgres, testing. Authored 2026-09-08 |
| M3 wk B (Docker→Azure CD, `insight-api`) | 📗 | ⬜ | Days 6–10: async, Dockerfile, Azure Container Apps + OIDC CD, project. `docs/AZURE_SETUP_GUIDE.md` authored |
...
| 2026-08-18 | Learner feedback: Day 8 (pytest/CI) too thin to actually do the project. Rewrote `16_theory_pytest_and_github_actions.md` from 206 → ~1300 lines: pytest setup/imports (`pip install -e`), reading failures, CLI flags, fixtures & `conftest.py`, unit vs integration (+Decimal/approx, parametrize, raises), a worked walkthrough of the Milestone-3/4 test, ruff, YAML crash course, both workflows annotated (service containers, health checks, env), secrets, cron gotchas, badges, a red-CI debugging playbook, a 7-step hands-on lab, and interview answers. Cross-linked from module README (Day 8 now ~3h + lab) and PROJECT_GUIDE milestones 2/4/5. **Convention going forward:** a bridge/theory lesson must cover every skill its project's milestones assume. |
| 2026-08-18 | Added `17_testing_and_ci_practice.ipynb` (Day 8 practice notebook, 78 cells): writes real test files into a git-ignored `sandbox/` and runs real pytest/ruff as subprocesses — first test, reading failures, flags, fixtures/`conftest.py`/scope, parametrize/raises/Decimal, skip-politely, ruff (against the project's own config), parsing the real workflow YAML, cron→local time, 3 exercises with solutions, and a graduation cell that runs the real project suite. Verified by headless execution: 78/78 cells, 0 exceptions. **Bug found & fixed while authoring:** the project failed `ruff check .` under current ruff (new default rules) — pinned `[tool.ruff.lint] select` in `pyproject.toml` and fixed the genuine `zip(..., strict=True)` finding in `fetch.py`; project is now ruff-clean and 6/6 green. |
| 2026-09-08 | Learner feedback after Modules 1–2: **(a)** syntax was under-explained, SQL especially; **(b)** material was understandable but forgettable. Both are now standing authoring rules in `CLAUDE.md`: break every construct down token by token on first appearance, and build retrieval scaffolding into every lesson (predict-first openers, "Remember this" boxes, recall questions before answers, a spaced-review section re-testing earlier days, and a per-module cheat sheet). Also rewrote `CLAUDE.md` with the `nl-open-data-pipeline` architecture and the real command set. Module 2 marked learned. |
| 2026-09-08 | **Authored Module 3 in full (both weeks).** 8 theory docs + 8 practice notebooks (Days 1–8), 4 exercise notebooks, the `insight-api` project, `docs/AZURE_SETUP_GUIDE.md` (944 lines, written from verified 2026 Microsoft Learn sources), `CHEATSHEET.md`, and a route-style module README. Project: FastAPI over the Module 2 warehouse — typed settings, `lifespan` engine, injected connections, response-model allow-lists, bound parameters + identifier allow-list, liveness/readiness split, multi-stage non-root Dockerfile, two-job CI (unit + a Postgres service container, failing the build if integration tests skip) and OIDC CD. **Verified:** every notebook executes top-to-bottom via nbconvert with 0 exceptions; every `<details>` solution executed; `ruff` clean; 25 unit tests pass and all 6 integration tests pass against a real Postgres; the image builds, runs as non-root, and serves live data. Running metaphor extended from Module 2: restaurant → food truck → depot → festival ground → delivery driver → visitor badge. |
| 2026-09-08 | **Bugs found and fixed while authoring** (each would have taught the wrong thing): the Day 6 blocking-event-loop demo did not reproduce under `TestClient` (each request gets its own loop) — rewritten to run a real uvicorn server, now shows an unrelated endpoint going 9x slower; the Day 7 cache-order demo silently failed because a trailing `#` comment after `COPY . .` is parsed as arguments, not a comment (now fixed *and* taught); the Day 8 workflow checker was fooled by a header comment mentioning `needs: test`, so it now strips comments before checking; `ruff`'s B008 flags FastAPI's `Depends()`/`Query()` defaults, so `extend-immutable-calls` is pinned in `pyproject.toml` with an explanatory comment; `starlette` deprecates `httpx` for `TestClient`, so the project depends on `httpx2`. **Most serious:** the project's `docker/docker-compose.yml` inherited the Compose project name `docker` from its parent directory — the same name Module 2's compose file gets — so the Module 3 stack silently attached **Module 2's data volume** (the seed never ran, and a `docker compose down -v` would have destroyed the learner's Module 2 database). Fixed with an explicit `name:` and a dedicated named volume, verified by tearing down and confirming Module 2's volume survived. **Convention going forward:** a notebook demo must be executed and its output read before shipping — "it runs without erroring" is not the same as "it demonstrates the claim". |
